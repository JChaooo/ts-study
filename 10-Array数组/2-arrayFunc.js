 ...
 * 将数组元素计算为一个值（从右到左）。
 */
var total = [9, 3, 2, 1, 2].reduceRight(function (previousValue, currentValue) {
    return (previousValue - currentValue);
});
 ...
 * 检测数组元素中是否有元素符合指定条件。
 */
var result = [1, 2, 3, 4, 5, 6].some(function (number, index, array) {
    return number > 6;
});
console.log(result); // 输出 false
/**
 ...
 * 对数组的元素进行排序。
 */
var new_arr = [9, 34, 0, 4, 83, 23, 743].sort(function (a, b) {
    return a - b;
});
console.log(new_arr);
/**
